package OpenSearch::Index::Alias;
use strict;
use warnings;
...
with 'OpenSearch::Parameters::IndexAlias';
...
sub get_mappings_p($self) {
  die("Parameter: alias is required.\n") if ( !$self->alias );
  return ( $self->base->_get( $self, [ $self->alias ] ) );
}

sub get_mappings($self) {
  my ($res);
  $self->get_mappings_p->then( sub { $res = shift; } )->wait;
  return $res;
}
...
sub create($self) {
  my $params = {
    optional => {
      url  => [qw/cluster_manager_timeout timeout/],
      body => []
    },
    required => {
      body => [qw/actions/]
  ...
  return ( $self->base->_post( $self, ['_aliases'], $params ) );

}

sub create_p($self) {
  my ($res);
  $self->create->then( sub { $res = shift; } )->wait;
  return $res;
}

sub delete($self) {
  die("Parameter: alias is required.\n") if ( !$self->alias );
  return ( $self->base->_post( $self, [ $self->index, '_alias', $self->alias ] ) );
}

sub delete_p($self) {
  my ($res);
  $self->delete->then( sub { $res = shift; } )->wait;
  return $res;
}
